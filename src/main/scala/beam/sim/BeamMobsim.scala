...
class BeamMobsim @Inject()(
  ...
  val rideHailingHouseholds: mutable.Set[Id[Household]] =
  ...
  private class MasterActor extends Actor with ActorLogging {
    ...
    if (beamServices.beamConfig.beam.debug.memoryConsumptionDisplayTimeoutInSec > 0) {
      memoryLoggingTimerActorRef =
        context.actorOf(Props(classOf[MemoryLoggingTimerActor]))
      memoryLoggingTimerCancellable = prepareMemoryLoggingTimerActor(
        beamServices.beamConfig.beam.debug.memoryConsumptionDisplayTimeoutInSec,
        context.system,
        memoryLoggingTimerActorRef)
    }
    ...
    private val rideHailingManager =
      context.actorOf(RideHailingManager.props(beamServices,
                                               scheduler,
    ...
    private val numRideHailAgents = math.round(
      scenario.getPopulation.getPersons.size * beamServices.beamConfig.beam.agentsim.agents.rideHailing.numDriversAsFractionOfPopulation)
    private val rideHailingVehicleType = scenario.getVehicles.getVehicleTypes
    ...
    scenario.getPopulation.getPersons
      ...
      .forEach { person =>
        ...
        val rideHailId =
          Id.create(rideHailingName, classOf[RideHailingAgent])
        val rideHailVehicleId = Id.createVehicleId(
          s"rideHailingVehicle-person=${person.getId}") // XXXX: for now identifier will just be initial location (assumed unique)
        val rideHailVehicle: Vehicle = VehicleUtils.getFactory
          .createVehicle(rideHailVehicleId, rideHailingVehicleType)
        val rideHailingAgentPersonId: Id[RideHailingAgent] =
          Id.createPersonId(rideHailingName)
        val information =
        ...
        val rideHailBeamVehicle =
          new BeamVehicle(powerTrain, rideHailVehicle, vehicleAttribute, Car)
        beamServices.vehicles += (rideHailVehicleId -> rideHailBeamVehicle)
        rideHailBeamVehicle.registerResource(rideHailingManager)
        val rideHailingAgentProps =
          RideHailingAgent.props(beamServices,
                                 scheduler,
                                 transportNetwork,
                                 eventsManager,
                                 rideHailingAgentPersonId,
                                 rideHailBeamVehicle,
                                 rideInitialLocation)
        val rideHailingAgentRef: ActorRef =
          context.actorOf(rideHailingAgentProps, rideHailingName)
    ...
    Await.result(beamServices.beamRouter ? Broadcast(InitTransit(scheduler)),
                 timeout.duration)
