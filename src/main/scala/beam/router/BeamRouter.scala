...
class BeamRouter(services: BeamServices,
  ...
  private def initTransit(scheduler: ActorRef) = {
    def createTransitVehicle(transitVehId: Id[Vehicle],
      ...
      val vehicleTypeId = Id.create(
        mode.toString.toUpperCase + "-" + route.agency_id,
        classOf[VehicleType])
      ...
      val vehicleType =
        if (transitVehicles.getVehicleTypes.containsKey(vehicleTypeId)) {
          ...
          log.debug(
            s"no specific vehicleType available for mode and transit agency pair '${vehicleTypeId.toString})', using default vehicleType instead")
      ...
      mode match {
        case (BUS | SUBWAY | TRAM | CABLE_CAR | RAIL | FERRY)
            if vehicleType != null =>
    ...
    val activeServicesToday = transportNetwork.transitLayer
      .getActiveServicesForDate(services.dates.localBaseDate)
          ...
          case IndexedSeq(fromStopIdx, toStopIdx) =>
            ...
            if (config.transitOnStreetNetwork && isOnStreetTransit(mode)) {
              stopToStopStreetSegmentCache.getOrElseUpdate(
                (fromStop, toStop),
                routeTransitPathThroughStreets(fromStop, toStop)) match {
                case Some(streetSeg) =>
                   ...
                   vehicleId: Id[Vehicle]) =>
                    BeamPath(
                      ...
                      SpaceTime(startEdge.getGeometry.getStartPoint.getX,
                                ...
                                departureTime),
                      SpaceTime(endEdge.getGeometry.getEndPoint.getX,
                   ...
                   vehicleId: Id[Vehicle]) =>
                    BeamPath(
                      ...
                      services.geo.distLatLon2Meters(
                        new Coord(startEdge.getGeometry.getStartPoint.getX,
                                  startEdge.getGeometry.getStartPoint.getY),
                        new Coord(endEdge.getGeometry.getEndPoint.getX,
                                  endEdge.getGeometry.getEndPoint.getY)
                      )
                    )
              }
  ...
  private def routeTransitPathThroughStreets(fromStopIdx: Int,
    ...
    val fromVertex = transportNetwork.streetLayer.vertexStore.getCursor(
      transportNetwork.transitLayer.streetVertexForStop.get(fromStopIdx))
    val toVertex = transportNetwork.streetLayer.vertexStore.getCursor(
    ...
    val edgeIds: Vector[Int] = stopIdxs
      .map { stopIdx =>
        if (transportNetwork.transitLayer.streetVertexForStop.get(stopIdx) >= 0) {
          ...
          val split = transportNetwork.streetLayer.findSplit(stopVertex.getLat,
                                                             stopVertex.getLon,
                                                             10000,
                                                             StreetMode.CAR)
          if (split != null) {
...
object BeamRouter {
  ...
  /**
    ...
    */
  case class RoutingRequest(origin: Location,
