...
class R5RoutingWorker(val beamConfig: BeamConfig,
                      val geo: GeoUtils,
                      val dates: DateUtils,
  ...
  val distanceThresholdToIgnoreWalking =
    beamConfig.beam.agentsim.thresholdForWalkingInMeters // meters
      ...
      val travelTime = (time: Long, linkId: Int) =>
        maybeTravelTime match {
          case Some(matsimTravelTime) =>
            matsimTravelTime
              .getLinkTravelTime(network.getLinks.get(Id.createLinkId(linkId)),
                                 time.toDouble,
                                 null,
                                 null)
              .toLong
  ...
  case class R5Request(from: Coord,
                       to: Coord,
                       time: WindowTime,
                       directMode: LegMode,
                       accessMode: LegMode,
  ...
  def calcRoute(routingRequest: RoutingRequest): RoutingResponse = {
    ...
    def tripsForVehicle(vehicle: StreetVehicle): Seq[EmbodiedBeamTrip] = {
      ...
      val maybeWalkToVehicle: Option[BeamLeg] = if (mainRouteFromVehicle) {
        val time = routingRequest.departureTime match {
          case time: DiscreteTime =>
            WindowTime(time.atTime, beamConfig.beam.routing.r5.departureWindow)
          case time: WindowTime => time
        }
        if (geo.distInMeters(vehicle.location.loc, routingRequest.origin) > distanceThresholdToIgnoreWalking) {
          val from = geo.snapToR5Edge(transportNetwork.streetLayer,
                                      geo.utm2Wgs(routingRequest.origin),
                                      10E3)
          val to = geo.snapToR5Edge(transportNetwork.streetLayer,
                                    geo.utm2Wgs(vehicle.location.loc),
                                    10E3)
      ...
      val maybeUseVehicleOnEgress: Option[BeamLeg] = if (mainRouteToVehicle) {
        ...
        val estimateDurationToGetToVeh: Int = math
          .round(
            geo.distInMeters(routingRequest.origin, vehicle.location.loc) / 5.8)
          .intValue()
        val time = routingRequest.departureTime match {
          case time: DiscreteTime =>
            WindowTime(time.atTime + estimateDurationToGetToVeh,
                       beamConfig.beam.routing.r5.departureWindow)
        ...
        val from = geo.snapToR5Edge(transportNetwork.streetLayer,
                                    ...
                                    10E3)
        val to = geo.snapToR5Edge(transportNetwork.streetLayer,
                                  geo.utm2Wgs(routingRequest.destination),
                                  10E3)
      ...
      val time = routingRequest.departureTime match {
        case time: DiscreteTime =>
          WindowTime(time.atTime + walkToVehicleDuration,
                     beamConfig.beam.routing.r5.departureWindow)
        case time: WindowTime =>
      ...
      val tripsWithFares = profileResponse.options.asScala.flatMap(option => {
        ...
        option.itinerary.asScala
          ...
          .map(itinerary => {
            ...
            val tripStartTime = dates.toBaseMidnightSeconds(
              itinerary.startTime,
              transportNetwork.transitLayer.routes.size() == 0)
            ...
            if (isRouteForPerson && access.mode != LegMode.WALK) {
              if (!isTransit)
                legsWithFares = legsWithFares :+ (dummyWalk(
                  tripStartTime + access.duration), 0.0)
            }
             ...
             assuming that: For each transit in option there is a TransitJourneyID in connection
              ...
              segments.foreach {
                case (transitSegment, transitJourneyID) =>
                  ...
                  legsWithFares ++= segmentLegs.zipWithIndex.map(beamLeg =>
                    (beamLeg._1, if (beamLeg._2 == 0) fare else 0.0))
                  arrivalTime = dates.toBaseMidnightSeconds(
            ...
            TripWithFares(
              BeamTrip(legsWithFares.map(_._1), mapLegMode(access.mode)),
    ...
    if (!embodiedTrips.exists(_.tripClassifier == WALK)) {
      ...
      if (maybeBody.isDefined) {
        ...
        val dest = new Coord(routingRequest.destination.getX,
                             routingRequest.destination.getY)
        val beelineDistanceInMeters =
          geo.distInMeters(origin, dest)
        val bushwhackingTime = Math.round(
            ...
            EmbodiedBeamLeg(
              BeamLeg(
                ...
                BeamPath(
                  ...
                  SpaceTime(
                    dest,
                    routingRequest.departureTime.atTime + bushwhackingTime),
  ...
  private def buildStreetPath(segment: StreetSegment,
    ...
    BeamPath(
      activeLinkIds,
      None,
      SpaceTime(segment.geometry.getStartPoint.getX,
                segment.geometry.getStartPoint.getY,
                tripStartTime),
      SpaceTime(segment.geometry.getEndPoint.getX,
  ...
  private def getFareSegments(
      ...
      fromTime: ZonedDateTime): Vector[BeamFareSegment] = {
    ...
    val duration = ChronoUnit.SECONDS
  ...
  def getPlan(request: ProfileRequest): ProfileResponse = {
    ...
    if (request.hasTransit) {
      ...
      for (path <- usefullpathList.asScala) {
        profileResponse.addTransitPath(accessRouter.asJava,
                                       egressRouter.asJava,
                                       path,
                                       transportNetwork,
...
object R5RoutingWorker {
  def props(beamServices: BeamServices,
            ...
            tollCalculator: TollCalculator) =
    Props(
      new R5RoutingWorker(beamServices.beamConfig,
                          beamServices.geo,
                          beamServices.dates,
                          transportNetwork,
                          network,
